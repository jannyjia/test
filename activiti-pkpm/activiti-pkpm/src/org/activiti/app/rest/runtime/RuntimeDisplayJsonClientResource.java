...
//    }
  ...
  protected void processElements(
    ...
    for (FlowElement element : elementList) {
      ...
      Boolean isCompletedElement = false, 
    		  isCurrentElement = false;
      
      if (completedElements != null) {
    	  isCompletedElement = completedElements.contains(element.getId());
    	  elementNode.put("completed", isCompletedElement);
      }

      if (currentElements != null) {
    	  isCurrentElement = currentElements.contains(element.getId());
    	  elementNode.put("current", isCurrentElement);
      }
      
      //依然从记录表里找，看起来记录表里会同事存在当前正在执行的action
	  if((isCurrentElement || isCompletedElement) 
		  ...
		  if(historicActivityInstances.size() >= 1) {
    		  //为ElementNode生成额外属性
    		  ArrayNode hiActivitiArray = objectMapper.createArrayNode();
    		  
    		  for (HistoricActivityInstance obj : historicActivityInstances) {
    			  ObjectNode hiActivitiNode = objectMapper.createObjectNode();
    			  hiActivitiNode.put("assignee", obj.getAssignee());
    			  hiActivitiNode.put("end_time", obj.getEndTime() != null ? obj.getEndTime().toString():"");
    			  hiActivitiNode.put("start_time", obj.getStartTime() !=null ? obj.getStartTime().toString():"");
    			  hiActivitiNode.put("act_type", obj.getActivityType());
    			  
    			  hiActivitiArray.add(hiActivitiNode);
    		  }
    		  
    		  //设置到elementNode 里
    		  elementNode.put("hiActivitiArray", hiActivitiArray);
    	  }
	  }
