...
public class RestResponseFactory {
  ...
  public TaskResponse createTaskResponse(Task task, RestUrlBuilder urlBuilder) {
    ...
    if (response.getProcessDefinitionId() != null) {
      ...
      ProcessDefinition defInstance = ProcessEngines.getDefaultProcessEngine()
    		  .getRepositoryService().createProcessDefinitionQuery()
    		  .processDefinitionId(response.getProcessDefinitionId()).singleResult();
      if(defInstance != null) {
    	  response.setProcessDefinitionTitle(defInstance.getName());
      }
    }
    ...
    if (response.getProcessInstanceId() != null) {
      ...
      HistoricProcessInstance processInstance= ProcessEngines.getDefaultProcessEngine().
    		  ...
    		  processInstanceId(response.getProcessInstanceId()).singleResult();
      if(processInstance != null) {
    	  response.setProcessInstanceTitle(processInstance.getName());
    	  response.setProcessInstanceStartTime(processInstance.getStartTime());
    	  response.setProcessInstanceStartUser(processInstance.getStartUserId());
      }
    }
  ...
  public HistoricTaskInstanceResponse createHistoricTaskInstanceResponse(HistoricTaskInstance taskInstance, RestUrlBuilder urlBuilder) {
    ...
    if (taskInstance.getProcessDefinitionId() != null) {
      ...
      ProcessDefinition defInstance = ProcessEngines.getDefaultProcessEngine()
    		  .getRepositoryService().createProcessDefinitionQuery()
    		  .processDefinitionId(result.getProcessDefinitionId()).singleResult();
      if(defInstance != null) {
    	  result.setProcessDefinitionTitle(defInstance.getName());
      }
    }
    ...
    //设置流程标题、发起人和发起时间
      ...
      if(processInstance != null) {
    	  result.setProcessInstanceTitle(processInstance.getName());
    	  result.setProcessInstanceStartTime(processInstance.getStartTime());
    	  result.setEndTime(processInstance.getEndTime());
    	  result.setProcessInstanceStartUser(processInstance.getStartUserId());
      }
